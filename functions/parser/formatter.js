  ...
  printSets(sets) {
    ...
    for (const set of sets) {
      if (!set.hasSubPattern) {
        ...
        const shouldWrapInParen = sets.length > 1; // Surround the subpattern in parenthesis if there's more to come, otherwise omit
        const setName = this.printSets(set.pattern[0], set.pattern[0].length > 1).title;
        fullTitle += `${set.count} x (${setName})`;
        if (shouldWrapInParen) {
          fullTitle = `(${fullTitle})`;
        }
        fullTitle = `${fullTitle} + `; // Set up the ending for slice

        for (const subSet of set.pattern) {
          for (const component of subSet) {
            const setNameForAverage = this.determineSetName(component, false); // don't include parens
            const setAverage = this.determineSetAverage(component);
            const setSplits = this.determineSetDetails(component);
            fullDescription += `⏱️ ${setNameForAverage} ${setAverage}${setSplits === `` ? "" : "\n"}${setSplits}\n\n`;
          }
        }
      }
